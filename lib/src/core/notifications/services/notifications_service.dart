...
// import 'package:ahtizam/features/auth/regestration/application/auth_service.dart';
...
import 'package:ahtizam/src/core/notifications/models/notification_model.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';

import 'package:ahtizam/firebase_options.dart';
import 'package:ahtizam/src/constants/keys.dart';
import 'package:ahtizam/src/localization/current_language.dart';
import 'package:ahtizam/src/routing/app_router.dart';
...
class NotificationsService {
  ...
  Future<void> init() async {
    ...
    // Listen for ***REMOVED*** ***REMOVED*** and send to backend
  
    final userId =   _ref.read(userDataProvider.notifier).userinformation.email;
    _messaging.onTokenRefresh.listen((***REMOVED***) {
      _ref
          .read(deviceTokenControllerProvider.notifier)
          .sendFCMToken(***REMOVED***, userId);
    });
    // Also send the current ***REMOVED***
  ...
  /// Send FCM ***REMOVED*** to backend
  Future<void> sendDeviceToken(String userId) async {
    try {
      final ***REMOVED*** = await _messaging.getToken();
      if (***REMOVED*** != null) {
        debugPrint(' sending device ***REMOVED***:$***REMOVED***');
      ...
      debugPrint('Error sending device ***REMOVED***: $e\n$st');
    }
  }
...
/// Controller to send device ***REMOVED*** to backend
@riverpod
class DeviceTokenController extends _$DeviceTokenController {
  ...
  Future<void> sendFCMToken(String ***REMOVED***, String userId) async {
    ...
    // try {
    await repo.sendFCMToken(***REMOVED***, userId);
